...
7. **CANVAS** — If the turn produced a substantive summary or explanation, elevate it as the **current board** in the `<!-- BRAIN:CANVAS -->` region of `brain/index.html` (the `.whiteboard`): replace the live `<article class="board board--live">`, demote the previous current to the top of `.canvas-earlier` as a collapsed `<details class="board board--past">`, cap the stack at 5, update `canvas-count` + the `Earlier · N` label. Leave only a one-line `📋 … → on the canvas` pointer in the terminal. This is how the canvas stays current without relying on memory — it rides the enforced stamp.
