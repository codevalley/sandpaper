...
test('copyTree merge aborts on same-inode destination file byte and mode edits', {
  skip: process.platform === 'win32',
}, (t) => {
  const root = fixture(t, 'sandpaper-copy-merge-file-race-');
  const source = join(root, 'source');
  const destination = join(root, 'destination');
  const userFile = join(destination, 'user.md');
  mkdirSync(source);
  mkdirSync(destination);
  writeFileSync(join(source, 'generated.md'), 'generated\n');
  writeFileSync(userFile, 'original user bytes\n');
  chmodSync(userFile, 0o640);
  const originalInode = statSync(userFile).ino;

  const error = thrown(() => copyTree(source, destination, {
    overwriteNamespaced: false,
    sourceRoot: root,
    destinationRoot: root,
    hooks: {
      beforeStage() {
        writeFileSync(userFile, 'concurrent user bytes\n');
        chmodSync(userFile, 0o600);
      },
    },
  }));

  assert.match(error.message, /Could not commit Sandpaper destination tree/);
  assert.equal(statSync(userFile).ino, originalInode);
  assert.equal(readFileSync(userFile, 'utf8'), 'concurrent user bytes\n');
  assert.equal(statSync(userFile).mode & 0o777, 0o600);
  assert.equal(existsSync(join(destination, 'generated.md')), false);
});

test('copyTree merge aborts on same-inode destination root and directory mode edits', {
  skip: process.platform === 'win32',
}, (t) => {
  const root = fixture(t, 'sandpaper-copy-merge-directory-race-');
  const source = join(root, 'source');
  const destination = join(root, 'destination');
  ...
  mkdirSync(nested);
  writeFileSync(join(source, 'generated.md'), 'generated\n');
  writeFileSync(join(nested, 'user.md'), 'user\n');
  ...
  assert.equal(readFileSync(join(nested, 'user.md'), 'utf8'), 'user\n');
  assert.equal(existsSync(join(destination, 'generated.md')), false);
});

test('copyTree owned replacement aborts on same-inode destination edits', {
  skip: process.platform === 'win32',
}, (t) => {
  const root = fixture(t, 'sandpaper-copy-owned-file-race-');
  const source = join(root, 'source');
  const destination = join(root, 'destination');
  const ownedFile = join(destination, 'owned.md');
  mkdirSync(source);
  mkdirSync(destination);
  writeFileSync(join(source, 'owned.md'), 'new generated bytes\n');
  writeFileSync(ownedFile, 'old generated bytes\n');
  chmodSync(ownedFile, 0o644);
  const originalInode = statSync(ownedFile).ino;
  ...
  const error = thrown(() => copyTree(source, destination, {
    ...
    hooks: {
      beforeStage() {
        writeFileSync(ownedFile, 'concurrent owned edit\n');
  ...
  assert.match(error.message, /Could not commit Sandpaper destination tree/);
  assert.equal(statSync(ownedFile).ino, originalInode);
  assert.equal(readFileSync(ownedFile, 'utf8'), 'concurrent owned edit\n');
  assert.equal(statSync(ownedFile).mode & 0o777, 0o600);
});

test('copyTree rejects a destination tree that changes during its scan consistency check', (t) => {
  const root = fixture(t, 'sandpaper-copy-scan-consistency-');
  const source = join(root, 'source');
  const destination = join(root, 'destination');
  const userFile = join(destination, 'user.md');
  mkdirSync(source);
  ...
  assert.throws(() => copyTree(source, destination, {
    overwriteNamespaced: false,
    sourceRoot: root,
    destinationRoot: root,
    fs: {
      readdirSync(path, ...args) {
        if (path === destination && ++destinationScans === 2) {
          writeFileSync(userFile, 'changed between scans\n');
        }
        return readdirSync(path, ...args);
      },
    },
  }), /Sandpaper destination tree changed during preflight/);

  assert.equal(statSync(userFile).ino, originalInode);
  assert.equal(readFileSync(userFile, 'utf8'), 'changed between scans\n');
  assert.equal(existsSync(join(destination, 'generated.md')), false);
});
